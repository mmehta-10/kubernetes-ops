...
terraform {
  source = "github.com/ManagedKube/kubernetes-ops.git//terraform-modules/azure/kubernetes/external-secrets/secret_store?ref=v2.0.85"
}
...
inputs = {
  ...
  vault_url        = dependency.vault.outputs.azurerm_key_vault_uri

  oidc_k8s_issuer_url  = dependency.aks.outputs.oidc_issuer_url
  azure_tenant_id      = dependency.aks.outputs.tenant_id
  azurerm_key_vault_id = dependency.vault.outputs.azurerm_key_vault_id
}
